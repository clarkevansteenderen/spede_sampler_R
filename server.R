...
server = function(input, output, session) {
    ...
    if (!dir.exists(input$raw_file_path)) 
      ...
      ################################################################################################
        ...
        observeEvent(input$GMCY_oversplit_full_table,{
          output$GMYC_oversplit_table = renderTable(splitting_species_bound, rownames = FALSE, colnames = TRUE, digits = 2)
        })
        ...
        output$GMYC_oversplit_full_table_download = downloadHandler(
          ...
          content = function (file){write.csv(splitting_species_bound, file, row.names = FALSE)}
        )
        ...
        observeEvent(input$GMYC_oversplit_boxplot, {
          ...
          if (is.null( predefined_groups_uploaded() )) shinyalert::shinyalert("No grouping information uploaded", "You did not upload a .csv file with predefined grouping information for your sequences", type = "warning")
            ...
            if(nrow(splitting_species_bound) > 0){
              
              output$GMYC_oversplit_plot = renderPlot({
                ggplot(data = splitting_species_bound, aes(x = predef_unique, y = Freq)) + 
                  geom_boxplot() +
        ...
        observeEvent(input$view_summary_match_data, {
          ...
          if (is.null( predefined_groups_uploaded() )) shinyalert::shinyalert("No grouping information uploaded", "You did not upload a .csv file with predefined grouping information for your sequences", type = "warning")
            ...
            avg_split_ratio = mean(record$splitting_ratio)
            stdev_split_ratio = sd(record$splitting_ratio)
            min_split_ratio = min(record$splitting_ratio)
            max_split_ratio = max(record$splitting_ratio)
            split_ratio_summary = round( c(avg_split_ratio, stdev_split_ratio, min_split_ratio, max_split_ratio) ,2 )
            
            # stats for percentage of GMYC:predefined_species excluding all single sample representatives
            avg_split_ratio_excl_single = mean(record$splitting_excl_singles)
            stdev_split_ratio_excl_single = sd(record$splitting_excl_singles)
            min_split_ratio_excl_single = min(record$splitting_excl_singles)
            max_split_ratio_excl_single = max(record$splitting_excl_singles)
            split_ratio_summary_excl_single = round( c(avg_split_ratio_excl_single, stdev_split_ratio_excl_single, min_split_ratio_excl_single, max_split_ratio_excl_single) ,2 )
            
            
            # create a data frame housing summary statistics, called 'match_stats.df'
            match_stats.df = data.frame(matrix(nrow=4, ncol = 6)) 
            colnames(match_stats.df) = c("statistic", "percentage_matches", "percentage_matches_excl_single_samples", "percentage_single_sample_GMYC_species", "splitting_ratio", "splitting_ratio_excl_single_sample_spp")
            ...
            match_stats.df$splitting_ratio = split_ratio_summary
            match_stats.df$splitting_ratio_excl_single_sample_spp = split_ratio_summary_excl_single
  ...
  observe({
    ...
    # splitting RATIO DATA INC. SINGLETONS
    
    infile_summary_splitting_ratio_inc = input$summary_splitting_ratio_inc
    
    if (is.null(infile_summary_splitting_ratio_inc)) {
      return(NULL)
    }
    ...
    observeEvent(input$plot_summary_oversplits, {
      ...
      # splitting RATIO DATA EXCL SINGLETONS
      infile_summary_splitting_ratio_exc = input$summary_splitting_ratio_exc
      
      if (is.null(infile_summary_splitting_ratio_exc)) {
        return(NULL)
      }
      
      summary_splitting_exc_csv = read.csv(infile_summary_splitting_ratio_exc$datapath, header = T, check.names = F)
      summary_splitting_exc_csv_melt = reshape2::melt(summary_splitting_exc_csv)
      
      colnames(summary_splitting_inc_csv_melt) = c("file_name", "data_perc", "splitting_ratio")
      colnames(summary_splitting_exc_csv_melt) = c("file_name", "data_perc", "splitting_ratio_excl")
      
      summary_splitting_inc_csv_melt$splitting_excl = summary_splitting_exc_csv_melt$splitting_ratio_excl
      splitting_combo = reshape::melt( summary_splitting_inc_csv_melt )
      
      splitting_boxplot_summary <<- ggplot(data = splitting_combo, aes(x = data_perc, y = value)) + 
        geom_boxplot(aes(fill = variable)) +
        expand_limits(y = 0) +
        #scale_y_continuous(breaks = seq(0, max(splitting_combo$value), by = input$splitting_plot_summary_y_interval)) +
        stat_summary(fun = mean, aes(group = variable), geom="point", shape = as.numeric(input$splitting_plot_point_shape), size=input$splitting_plot_point_size, color=input$splitting_plot_point_col, position=position_dodge(0.77)) +
        scale_fill_manual(values=c(input$splitting_inc_col, input$splitting_exc_col), name = "", labels = c("+ singletons", "- singletons")) +
        xlab("Data Percentage") +
        ylab("Splitting ratio") +
        ggthemes[[input$ggtheme_summary_plots]] +
        theme(legend.position = c(input$splitting_plot_legend_x_pos, input$splitting_plot_legend_y_pos)) +
      ...
      output$summary_plot = renderPlot(splitting_boxplot_summary)
      multiple_summary_plots$splitting <<- splitting_boxplot_summary
  ...
  observe({
    ...
    # splitting PER MORPHOSPECIES GROUP
    infile_summary_splitting_morphospecies = input$summary_splitting_morphospecies
    
    if (is.null(infile_summary_splitting_morphospecies)) {
      return(NULL)
    }
    
    observeEvent(input$plot_summary_oversplits_per_morphospecies, {
      
      summary_oversplits_per_group_csv = read.csv(infile_summary_splitting_morphospecies$datapath, header = T, check.names = F)
      
      splitting_bar_summary <<- ggplot(data = summary_oversplits_per_group_csv, aes(x = predef_unique, y = Freq)) +
        geom_bar(stat = "summary", fill = input$splitting_morphospecies_col, col = "black") +
        expand_limits(y = 0) +
        xlab("Predefined group") +
        ylab("Mean splitting ratio per predefined \n group, on the full dataset (100%)") +
        ggthemes[[input$ggtheme_summary_plots]] +
        theme(axis.title.y = element_text(size = 14, margin = margin(t = 0, r = 20, b = 0, l = 0))) +
        theme(axis.title.x = element_text(size = 14, margin = margin(t = 20, r = 0, b = 0, l = 0))) +
        theme(axis.text.x = element_text(size = 12)) +
      ...
      output$summary_plot = renderPlot(splitting_bar_summary)
      multiple_summary_plots$splitting <<- splitting_bar_summary
      
    })
    
    # download the plot
    output$download_summary_splitting_morphospecies_plot <- downloadHandler(
      
      filename = function (){paste("splitting_predefined_groups_plot", input$plot_format_summary_splitting_morphospecies, sep = '.')},
      
      content = function (file){
        
        width.splitting_morphospecies_summary = as.numeric(input$w_plot_summary_splitting_morphospecies) 
        height.splitting_morphospecies_summary = as.numeric(input$h_plot_summary_splitting_morphospecies) 
        dpi.splitting_morphospecies_summary = as.numeric(input$res_plot_summary_splitting_morphospecies)
        units.splitting_morphospecies_summary = input$unit_plot_summary_splitting_morphospecies
        
        ggsave(file, width = width.splitting_morphospecies_summary, height = height.splitting_morphospecies_summary, dpi = dpi.splitting_morphospecies_summary, units = units.splitting_morphospecies_summary,
               splitting_bar_summary)
      }
    )
